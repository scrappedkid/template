...
import { LIST_PATH } from '@config/constants/paths'

const locationHelper = locationHelperBuilder({})

const history = createHistory()

const AUTHED_REDIRECT = '/home'
const UNAUTHED_REDIRECT = '/login'
...
export const UserIsAuthenticated = connectedRouterRedirect({
  ...
  authenticatedSelector: ({ firebase: { auth } }) => !auth.isEmpty && Boolean(auth.uid),
...
export function renderChildren (routes, match, parentProps) {
  return routes.map(route => (
    <Route key={`${match.url}-${route.path}`} path={`${match.url}/${route.path}`} render={(props) => <route.component {...parentProps} {...props} />} />)
  )
}
